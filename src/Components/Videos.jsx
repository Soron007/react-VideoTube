...
const Videos = () => {
    ...
    const [videoSrc, setVideoSrc] = useState(arrOfVideos[0].link);
    const [videoName, setVidoName] = useState(arrOfVideos[0].name)
            ...
            <Stack direction={['column', 'row']} h={'100vh'}>
                ...
                <VStack w={'full'}>
                    ...
                    <VStack alignItems={"flex-start"} p={'8'} w={'full'} overflowY={'auto'}>
                        ...
                        <Text>What the video is about</Text>
                    </VStack>
                </VStack>

                <VStack w={['full', 'xl']} alignItems={'stretch'} p={'8'} spacing={'8'} overflowY={'auto'}>
                    {
                        arrOfVideos.map((video, index, key) => (
                            <Button onClick={() => {
                                setVideoSrc(video.link)
                                setVidoName(video.name)
                            }}
                                key={video.name}
                                variant={"ghost"}
                                colorScheme={'purple'}>
